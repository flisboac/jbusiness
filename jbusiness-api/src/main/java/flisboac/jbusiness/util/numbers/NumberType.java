...
public enum NumberType {
	...
	private final Optional<Class<? extends Number>> nativeNumberClass;
	...
	private NumberType(boolean valid, 
		...
		this.nativeNumberClass = Optional.<Class<? extends Number>>ofNullable(nativeNumberClass);
	...
	private final BigDecimal generateLimitValue(String fieldName) {
		...
		if (!this.nativeNumberClass.isPresent()) {
		...
		Number number;
		try {
			field = this.nativeNumberClass.get().getField(fieldName);
